...
description: |
  ...
  is ready, asks about deploying, wants to push code up, or asks to create a PR. (rstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
For Design Review: run `source <(~/.claude/skills/rstack/bin/rstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** rstack /ship on <date>" \
...
Write to `~/.rstack/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
# Compute project slug for ~/.rstack/projects/ lookup
...
- Review report sections (`## RSTACK REVIEW REPORT`)
...
Check if the diff touches frontend files using `rstack-diff-scope`:

```bash
source <(~/.claude/skills/rstack/bin/rstack-diff-scope <base> 2>/dev/null)
```
...
- Message: "RStack recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
