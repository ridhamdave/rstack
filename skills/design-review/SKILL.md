...
description: |
  ...
  wants to polish the look of a live site. (rstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
**Find the rstack designer (optional — enables target mockup generation):**
...
REPORT_DIR=~/.rstack/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)
...
```
Write to: `~/.rstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
~/.rstack/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
If the rstack designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
eval "$(~/.claude/skills/rstack/bin/rstack-slug 2>/dev/null)" && mkdir -p ~/.rstack/projects/$SLUG
```
Write a one-line summary to `~/.rstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
