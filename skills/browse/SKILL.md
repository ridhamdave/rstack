...
description: |
  ...
  site", "take a screenshot", or "dogfood this". (rstack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
`$B` is the browse binary (resolved from `$_ROOT/.claude/skills/rstack/browse/dist/browse` or `~/.claude/skills/rstack/browse/dist/browse`).
