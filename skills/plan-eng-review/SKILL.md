...
description: |
  ...
  start coding — to catch architecture issues before implementation. (rstack)
...
Write to `~/.rstack/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
2. Write the prompt to `~/.rstack/tmp/plan-eng-outside-voice-prompt.txt`.
...
5. Write stderr to `~/.rstack/tmp/plan-eng-outside-voice-stderr.txt`.
...
```bash
mkdir -p ~/.rstack
echo '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}' >> ~/.rstack/review-log.jsonl
```
