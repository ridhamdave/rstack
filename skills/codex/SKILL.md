...
```bash
~/.claude/skills/rstack/bin/rstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
- **Detect skill-file rabbit holes.** After receiving Codex output, scan for signs
  that Codex got distracted by skill files: `rstack-config`, `rstack-update-check`,
  `SKILL.md`, or `skills/rstack`. If any of these appear in the output, append a
  warning: "Codex appears to have read rstack skill files instead of reviewing your
