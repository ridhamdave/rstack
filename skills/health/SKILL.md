...
Append one JSONL line to `~/.rstack/projects/$SLUG/health-history.jsonl`:
...
Read the last 10 entries from `~/.rstack/projects/$SLUG/health-history.jsonl` (if the
file exists and has prior entries).

```bash
eval "$(~/.claude/skills/rstack/bin/rstack-slug 2>/dev/null)" && mkdir -p ~/.rstack/projects/$SLUG
tail -10 ~/.rstack/projects/$SLUG/health-history.jsonl 2>/dev/null || echo "NO_HISTORY"
```
