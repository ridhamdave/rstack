...
description: |
  ...
  "let another agent use my browser", or "give browser access". (rstack)
...
Your rstack browser runs a local HTTP server. This skill creates a one-time setup key,
...
side panel if you have RStack Browser open."
...
credentials are written to `~/.openclaw/skills/rstack/browse-remote.json`.
...
to `~/.codex/skills/rstack/browse-remote.json`.
...
`~/.cursor/skills/rstack/browse-remote.json`.
