...
description: |
  ...
  design system or DESIGN.md. (rstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
**Find the rstack designer (optional — enables AI mockup generation):**
...
This phase generates visual previews of the proposed design system. Two paths depending on whether the rstack designer is available.
...
```bash
eval "$(~/.claude/skills/rstack/bin/rstack-slug 2>/dev/null)"
_DESIGN_DIR=~/.rstack/projects/$SLUG/designs/design-system-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
